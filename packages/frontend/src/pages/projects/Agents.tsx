...
const EMPTY: CreateAgentProfilePayload = { name: '', role: 'DEV_AGENT', skillSet: [], supportedPhases: [] };
...
export function AgentsPage() {
  ...
  async function handleSeed() {
    setSeeding(true);
    try {
      await agentsService.seedDefaults(projectId!);
      refetch();
  ...
  function openEdit(p: AgentProfile) {
    ...
    setSkillInput(p.skillSet.join(', '));
    setPhaseInput(p.supportedPhases.join(', '));
    setFormError('');
    setModalOpen(true);
  }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      if (editingId) {
        await agentsService.updateProfile(projectId!, editingId, payload);
      } else {
        await agentsService.createProfile(projectId!, payload);
      }
      setModalOpen(false);
      refetch();
    } catch (err) {
  ...
  async function handleDelete() {
    if (!deleteId) return;
    await agentsService.deleteProfile(projectId!, deleteId);
    setDeleteId(null);
    refetch();
  }
  ...
  async function addMapping(e: React.FormEvent) {
    e.preventDefault();
    if (!mappingProfileId || !mappingPhaseId) return;
    setMapSaving(true);
    try {
    ...
    <div className="space-y-8">
      ...
      {!loading && !profiles?.length ? (
        <EmptyState
          ...
          description="Seed default profiles (BA, Dev, QA, DevOps) or create your own."
      ...
      <div>
        ...
        <form onSubmit={addMapping} className="flex items-end gap-3 mb-4 flex-wrap">
          ...
          <Select
            ...
            className="w-56"
          />
          <Button type="submit" size="sm" loading={mapSaving}
            disabled={!mappingPhaseId || !mappingProfileId}>
            Add mapping
          </Button>
        </form>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border-default">
                ...
                <th className="text-left px-4 h-10 text-[11px] uppercase tracking-wide text-text-secondary font-medium">Agent</th>
                <th className="text-left px-4 h-10 text-[11px] uppercase tracking-wide text-text-secondary font-medium">Role</th>
                <th className="text-left px-4 h-10 text-[11px] uppercase tracking-wide text-text-secondary font-medium w-20"></th>
              </tr>
            </thead>
            <tbody>
              {!(mappings?.length) ? (
                <tr><td colSpan={4} className="px-4 py-6 text-center text-sm text-text-secondary">
                ...
                return (
                  <tr key={m.id} className="border-b border-border-subtle last:border-0">
                    ...
                    <td className="px-4 h-10">
                      <Badge variant={roleColors[m.agentProfile.role] ?? 'neutral'}>
                        {m.agentProfile.role.replace('_AGENT', '')}
                      </Badge>
                    </td>
                    <td className="px-4 h-10">
                      <Button size="sm" variant="danger"
                        onClick={() => agentsService.deleteMapping(projectId!, m.id).then(refetchMappings)}>
                        ✕
                      </Button>
                    </td>
                  </tr>
      ...
      >
        ...
        <form id="agent-form" onSubmit={handleSave} className="space-y-4">
          <Input label="Agent name" value={form.name}
            onChange={(e) => setForm({ ...form, name: e.target.value })} required />
          <Select label="Role" value={form.role} options={ROLES}
            onChange={(e) => setForm({ ...form, role: e.target.value })} />
          <TextArea label="Description" rows={2} value={form.description ?? ''}
            onChange={(e) => setForm({ ...form, description: e.target.value })} />
          <Input label="Skill set (comma separated)" value={skillInput}
            onChange={(e) => setSkillInput(e.target.value)}
            placeholder="e.g. code-generation, unit-testing" />
          <Input label="Supported phases (comma separated)" value={phaseInput}
            onChange={(e) => setPhaseInput(e.target.value)}
            placeholder="e.g. In Dev, In Review" />
        </form>
      </Modal>

      <ConfirmModal open={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={handleDelete}
        title="Delete Agent Profile"
        message="This agent profile and all its phase mappings will be deleted." />
    </div>
  );
}
